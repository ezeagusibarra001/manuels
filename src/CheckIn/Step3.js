    ...
    const HandleSubmit = async () => {
        ...
        if(image!==null){
            ...
            .then((res) => {
                ...
                bar3.addEventListener("transitionend",()=>{tres.style.backgroundColor="#a06ab4";setIcon3(true); setStep(step+1)})
            })
            .catch((err) => {
        ...
        }else{addToast("Debes adjuntar el comprobante de pago", {appearance: "warning",autoDismiss: true,});}
    }
    return(
        <div className="ContainerStep3">
            ...
            <div className="ChechDatosContainer">
                <div className="ScrollContent">
                    ...
                    <p> <img alt="img" src="../assets/iconoc9.webp" className="WelcomeIconos"/>Te has inscripto en la clase: <b>{currentClase[0].title}     </b></p>
                    <p> <img alt="img" src="../assets/iconoc9.webp" className="WelcomeIconos"/>Fecha:    <b>{dayjs(currentClase[0].date).format('DD/MM/YY')}</b></p>
                    <p> <img alt="img" src="../assets/iconoc9.webp" className="WelcomeIconos"/>Nombre:   <b>{CheckForm.name}                                </b></p>
                    <p> <img alt="img" src="../assets/iconoc9.webp" className="WelcomeIconos"/>Nombre:   <b>{CheckForm.lastname}                                </b></p>
                    <p> <img alt="img" src="../assets/iconoc9.webp" className="WelcomeIconos"/>E-mail:   <b>{CheckForm.mail}                                </b></p>
                    <p> <img alt="img" src="../assets/iconoc9.webp" className="WelcomeIconos"/>Telefono: <b>{CheckForm.phone}                               </b></p>
                    <p> <img alt="img" src="../assets/iconoc9.webp" className="WelcomeIconos"/>Monto Transferido: <b>${currentClase[0].price}</b>              </p>
                </div>
            </div>
            <p className="TextComprobante">Adjuntar comprobante:</p>
            <input className="CheckComprobante"
                    type="file"
                    name="file"
                    onChange={(e) => handleFile(e)}
                />
            <button className="ButtonConfirm" onClick={HandleSubmit}>Confirmar inscripción</button>
        </div>
    )
}
